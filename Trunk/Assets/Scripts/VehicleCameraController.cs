...
public class VehicleCameraController : MonoBehaviour
	...
	void DoSmoothFollow ()
		{
		if (target == null) return;
			float abc = Time.deltaTime;
			if (abc == 0)
				abc = 1;
			Vector3 updatedVelocity = (target.position - m_smoothLastPos) / abc;
		m_smoothLastPos = target.position;
	...
	void InitializeMouseOrbit ()
		...
		public void DoMouseOrbit ()
		{
			if (mode == Mode.MouseOrbit  ) 
			{
				...
				//m_transform.rotation=m_transform.rotation*Quaternion.Euler(m_orbitY,m_orbitX,0);
				m_transform.position = target.position + m_transform.rotation * new Vector3 (0.0f, 0.0f, -m_orbitDistance);
			}
		}
		public void myDargClick(){
			if (mode == Mode.MouseOrbit) {
